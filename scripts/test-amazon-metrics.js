...
async function main() {
  ...
  const acrossDates = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 90, sessions: 3000, buy_box_pct: 30, total_order_items: 90, ordered_revenue: 900, page_views: 4000, units_refunded: 0 },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-11', currency: 'GBP', units_ordered: 10, sessions: 20, buy_box_pct: 95, total_order_items: 10, ordered_revenue: 100, page_views: 25, units_refunded: 0 },
  ]
  ...
  // -------------------------------------------------------------------------
  const withNulls = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 5, sessions: 100, buy_box_pct: null, total_order_items: 5, ordered_revenue: 50, page_views: 120, units_refunded: 1 },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 5, sessions: 100, buy_box_pct: 80, total_order_items: 5, ordered_revenue: 50, page_views: 120, units_refunded: 0 },
  ]
  ...
  const zeroSessions = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 0, sessions: 0, buy_box_pct: null, total_order_items: 0, ordered_revenue: 0, page_views: 0, units_refunded: 0 },
  ]
  ...
  ok('order follows the allowlist (UK before DE), not size', r5.marketplaces[0].short === 'UK')

  // -------------------------------------------------------------------------
  console.log('\nSparse coverage (backfill still running)')
  // -------------------------------------------------------------------------
  const sparse = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 5, sessions: 50, ordered_revenue: 50, total_order_items: 5, page_views: 60, buy_box_pct: 90, units_refunded: 0 },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-14', currency: 'GBP', units_ordered: 5, sessions: 50, ordered_revenue: 50, total_order_items: 5, page_views: 60, buy_box_pct: 90, units_refunded: 0 },
  ]
  ...
  const statusRows = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', status: 'ok' },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-11', status: 'ok' },   // zero-sales day
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-12', status: 'gap' },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-13', status: 'parse_failed' },
    // 2026-08-14 deliberately absent -> never attempted
  ]
  const oneDayOfRows = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 10, sessions: 100, ordered_revenue: 100, total_order_items: 10, page_views: 120, buy_box_pct: 90, units_refunded: 0 },
  ]
  ...
  const fractionish = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 1, sessions: 10, ordered_revenue: 1, total_order_items: 1, page_views: 10, buy_box_pct: 0.9, units_refunded: 0 },
  ]
  ...
  const over100 = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 5, sessions: 1, ordered_revenue: 5, total_order_items: 5, page_views: 1, buy_box_pct: 100, units_refunded: 0 },
  ]
  ...
  // -------------------------------------------------------------------------
  console.log('\nMarketplace scope — explicit allowlist, fixed order')
  // -------------------------------------------------------------------------
  const SA = 'A17E79C6D8DWNP'   // dropped; ~725 days of legacy rows remain
  const UK = 'A1F83G8C2ARO7P'
  const DE = 'A1PA6795UKMFR9'

  ok('allowlist is exactly 12', mod.ALLOWED_MARKETPLACES.length === 12)
  ok('allowlist order is UK, US, CA, DE, FR, NL, PL, BE, IT, ES, IE, SE',
    mod.ALLOWED_MARKETPLACES.map(id => mod.marketplaceShort(id)).join(',') === 'UK,US,CA,DE,FR,NL,PL,BE,IT,ES,IE,SE')
  ok('Saudi Arabia is out of scope', mod.isAllowedMarketplace(SA) === false)
  ok('Turkey is out of scope', mod.isAllowedMarketplace('A33AVAJ2PDY3EV') === false)
  ok('UK is in scope', mod.isAllowedMarketplace(UK) === true)

  const withSA = [
    { marketplace_id: UK, report_date: '2026-08-10', currency: 'GBP', units_ordered: 10, sessions: 100, ordered_revenue: 100, total_order_items: 10, page_views: 120, buy_box_pct: 90, units_refunded: 0 },
    { marketplace_id: SA, report_date: '2026-08-10', currency: 'SAR', units_ordered: 999, sessions: 9999, ordered_revenue: 9999, total_order_items: 999, page_views: 9999, buy_box_pct: 99, units_refunded: 0 },
  ]
  const rs = aggregate(withSA, { start: '2026-08-10', end: '2026-08-10', grain: 'daily' })
  ok('out-of-scope marketplace produces no row', rs.marketplaces.length === 1)
  ok('the surviving row is UK', rs.marketplaces[0].marketplace_id === UK)
  ok('out-of-scope units excluded from the report', rs.marketplaces[0].current.units === 10)
  ok('discarded rows are counted, not silently dropped', rs.meta.rows_out_of_scope === 1)
  ok('scope is stated in meta', rs.meta.scope.length === 12)

  // A dropped marketplace with legacy STATUS rows must not be resurrected as a
  // zero-sales row — that path exists specifically to keep real marketplaces
  // visible, and would otherwise reintroduce SA through the back door.
  const saStatus = aggregate([], {
    start: '2026-08-10', end: '2026-08-10', grain: 'daily',
    status: [{ marketplace_id: SA, report_date: '2026-08-10', status: 'ok' }],
  })
  ok('legacy status rows do not resurrect a dropped marketplace', saStatus.marketplaces.length === 0)

  // Display order follows the allowlist, not size.
  const unordered = [
    { marketplace_id: 'A2NODRKZP88ZB9', report_date: '2026-08-10', currency: 'SEK', units_ordered: 5000, sessions: 9000, ordered_revenue: 5000, total_order_items: 5000, page_views: 9000, buy_box_pct: 90, units_refunded: 0 },
    { marketplace_id: UK, report_date: '2026-08-10', currency: 'GBP', units_ordered: 1, sessions: 10, ordered_revenue: 1, total_order_items: 1, page_views: 10, buy_box_pct: 90, units_refunded: 0 },
    { marketplace_id: DE, report_date: '2026-08-10', currency: 'EUR', units_ordered: 50, sessions: 500, ordered_revenue: 50, total_order_items: 50, page_views: 500, buy_box_pct: 90, units_refunded: 0 },
  ]
  const ro = aggregate(unordered, { start: '2026-08-10', end: '2026-08-10', grain: 'daily' })
  ok('order is the allowlist order, not by size',
    ro.marketplaces.map(m => m.short).join(',') === 'UK,DE,SE',
    `got ${ro.marketplaces.map(m => m.short).join(',')} — SE is largest but must come last`)

  // -------------------------------------------------------------------------
  console.log('\nIncomplete coverage reads as incomplete, not as zero')
  // -------------------------------------------------------------------------
  const { ratePresentation } = mod
  ok('complete coverage shows a real zero', ratePresentation(0, { complete: true }).state === 'ok')
  ok('partial coverage withholds a zero rate', ratePresentation(0, { complete: false }).state === 'incomplete')
  ok('partial coverage still shows a non-zero rate', ratePresentation(0.0495, { complete: false }).state === 'partial')
  ok('a null rate stays null', ratePresentation(null, { complete: false }).state === 'none')
  ok('missing coverage is treated as incomplete', ratePresentation(0, undefined).state === 'incomplete')

  // The Germany case: sessions but no units, over partial coverage.
  const dePartial = aggregate(
    [{ marketplace_id: DE, report_date: '2026-08-10', currency: 'EUR', units_ordered: 0, sessions: 65, ordered_revenue: 0, total_order_items: 0, page_views: 70, buy_box_pct: 90, units_refunded: 0 }],
    { start: '2026-08-10', end: '2026-08-14', grain: 'daily',
      status: [{ marketplace_id: DE, report_date: '2026-08-10', status: 'ok' }] })
  const de = dePartial.marketplaces[0]
  ok('DE conversion is arithmetically zero', de.current.conversion === 0)
  ok('DE coverage is partial', de.current.coverage.complete === false)
  ok('so the rate presents as incomplete, not 0.00%',
    ratePresentation(de.current.conversion, de.current.coverage).state === 'incomplete')

  // Same figures with every day confirmed pulled: now it IS a real zero.
  const deComplete = aggregate(
    [{ marketplace_id: DE, report_date: '2026-08-10', currency: 'EUR', units_ordered: 0, sessions: 65, ordered_revenue: 0, total_order_items: 0, page_views: 70, buy_box_pct: 90, units_refunded: 0 }],
    { start: '2026-08-10', end: '2026-08-10', grain: 'daily',
      status: [{ marketplace_id: DE, report_date: '2026-08-10', status: 'ok' }] })
  ok('with full coverage the same zero is reported as zero',
    ratePresentation(deComplete.marketplaces[0].current.conversion, deComplete.marketplaces[0].current.coverage).state === 'ok')

  // -------------------------------------------------------------------------
  console.log('\nKnown-bad data flag (status cannot detect wrong-account pulls)')
  // -------------------------------------------------------------------------
  // DE's entry was removed 2026-08-25 — the wrong-account rows were deleted
  // from amazon_asin_daily, not re-pulled over the top of them, so there's
  // nothing left for the flag to warn about. SUSPECT_DATA is empty for now;
  // these assertions check the mechanism stays inert, not dead.
  ok('SUSPECT_DATA is empty — no marketplace currently flagged', Object.keys(mod.SUSPECT_DATA).length === 0)
  ok('DE is not flagged — its bad rows were deleted, not re-pulled over', mod.suspectDataFor(DE, '2026-08-01') === null)
  ok('UK is not flagged', mod.suspectDataFor(UK, '2026-08-01') === null)
  ...
  const compared = [
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-10', currency: 'GBP', units_ordered: 100, sessions: 1000, ordered_revenue: 1000, total_order_items: 100, page_views: 1200, buy_box_pct: 90, units_refunded: 0 },
    { marketplace_id: 'A1F83G8C2ARO7P', report_date: '2026-08-03', currency: 'GBP', units_ordered: 50, sessions: 1000, ordered_revenue: 500, total_order_items: 50, page_views: 1100, buy_box_pct: 80, units_refunded: 0 },
  ]
