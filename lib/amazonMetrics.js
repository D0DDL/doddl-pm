...
// ─────────────────────────────────────────────────────────────────────────────
//
// The report shows these twelve marketplaces, in this order, and nothing else.
// This is an explicit allowlist rather than "whatever is in the table" because
// amazon_asin_daily retains rows for marketplaces doddl no longer sells in —
// Saudi Arabia has ~725 days of legacy rows from before it was dropped. Those
// rows stay in the database; they must not reach the report.
//
// Out-of-scope marketplaces are ignored ENTIRELY: no row, not rendered as zero,
// excluded from every total, and excluded from the data-coverage bounds. The
// last one matters — SA's legacy history would otherwise drag the reported
// "data starts" date years earlier than the in-scope data actually goes.
//
// Order here IS display order. It is deliberately not a ranking by revenue or
// units: a fixed running order makes the table scannable week to week, and stops
// rows jumping around as trade shifts.
...
const ALLOWED_SET = new Set(ALLOWED_MARKETPLACES)
const ORDER_INDEX = new Map(ALLOWED_MARKETPLACES.map((id, i) => [id, i]))

export const isAllowedMarketplace = (id) => ALLOWED_SET.has(id)
export const marketplaceOrder = (id) => (ORDER_INDEX.has(id) ? ORDER_INDEX.get(id) : Number.MAX_SAFE_INTEGER)

// ─────────────────────────────────────────────────────────────────────────────
// Known-bad data
// ─────────────────────────────────────────────────────────────────────────────
//
// TEMPORARY. amazon_asin_daily_status records whether a FETCH SUCCEEDED, not
// whether what came back is correct. A marketplace pulled against the wrong
// seller account returns status='ok' with plausible-looking rows, so coverage
// logic marks it complete and the report states its figures with full
// confidence. No coverage rule can detect this — it is a provenance problem,
// not a completeness one.
//
// Entries here mark a marketplace's rows as unreliable up to a cutoff date, so
// the report says so instead of quietly presenting them as fact. REMOVE an entry
// once that marketplace has been re-pulled with the correct account.
//
// DE's entry (wrong-seller-account rows through 2026-08-24) removed 2026-08-25
// — the affected rows were deleted from amazon_asin_daily directly, not
// re-pulled over the top of them, so there's nothing left in the table for
// this flag to be warning about. Empty for now; the mechanism stays for the
// next time this class of problem shows up somewhere else.
export const SUSPECT_DATA = {}

export function suspectDataFor(id, rangeStart) {
  const s = SUSPECT_DATA[id]
  if (!s) return null
  // Only flag if the requested range actually overlaps the affected period.
  return rangeStart <= s.through ? s : null
}
...
/**
 * Should a derived rate be presented as a number, or as "incomplete"?
 *
 * A rate is a ratio of two sums. Both sums only cover the days that were
 * actually pulled, so with partial coverage the rate describes a subset of the
 * period while looking exactly like a figure for the whole of it.
 *
 * Zero is the case that actually misleads. `0.00%` conversion reads as "we got
 * traffic and sold nothing" — a real, actionable business fact. But a zero
 * numerator over partial coverage is indistinguishable from "the days with the
 * sales on them have not been pulled yet". Germany showed exactly this: 65
 * sessions, 0 units, rendered as a confident 0.00%.
 *
 * So:
 *   complete coverage            -> show the rate
 *   partial coverage, non-zero   -> show the rate, marked as partial
 *   partial coverage, zero       -> show "incomplete", never "0.00%"
 *
 * The middle case still shows a number because a rate over 26 of 28 days is
 * useful and suppressing it would be its own kind of dishonesty. Only the
 * genuinely ambiguous case is withheld.
 */
export function ratePresentation(value, coverage) {
  const complete = !!coverage?.complete
  if (value == null) return { value: null, state: 'none' }
  if (complete) return { value, state: 'ok' }
  if (value === 0) return { value: 0, state: 'incomplete' }
...
export function aggregate(rows, { start, end, grain = 'daily', compare = null, status = [] }) {
  ...
  for (const srow of status || []) {
    ...
    if (!isAllowedMarketplace(srow.marketplace_id)) continue
  ...
  for (const row of rows) {
    ...
    // Out-of-scope marketplaces are dropped before anything is accumulated, so
    // they cannot reach a row, a total, or the coverage figures. Filtered at the
    // query layer too (lib/amazonSupabase.js); this is the backstop that holds
    // even if a caller passes unfiltered rows.
    if (!isAllowedMarketplace(row.marketplace_id)) {
      outOfScope += 1
      continue
    }
  ...
  for (const srow of status || []) {
    if (srow?.status !== 'ok') continue
    if (!currentDates.has(srow.report_date)) continue
    // Without this guard a dropped marketplace with legacy status rows would be
    // resurrected here as a zero-sales row — exactly what the allowlist exists
    // to prevent.
    if (!isAllowedMarketplace(srow.marketplace_id)) continue
    ensure(srow.marketplace_id)
  }
  ...
  const marketplaces = [...byMarketplace.values()]
    ...
    // Fixed running order from ALLOWED_MARKETPLACES, not a ranking. The table is
    // read week to week, so rows staying put matters more than seeing the
    // biggest market first — and a revenue ranking was never sound anyway across
    // currencies with no FX table.
    .sort((a, b) => marketplaceOrder(a.marketplace_id) - marketplaceOrder(b.marketplace_id))
  ...
  return {
    ...
    meta: {
      ...
      rows_out_of_scope: outOfScope,
      scope: ALLOWED_MARKETPLACES,
      scope_note: `Report scope is fixed to ${ALLOWED_MARKETPLACES.length} marketplaces. Rows for any other marketplace in amazon_asin_daily are ignored entirely — not shown, not counted, not included in coverage.`,
      pct_scale: pctScale,
