...
// Two-phase audit logging to make rate-limit accounting race-free:
//   1. insertInFlight — writes row with status_code=0 BEFORE the handler runs.
//      Concurrent burst requests each see their peers' rows in the count.
//   2. finalizeInFlight — patches the row with the real status + response body
//      once the handler completes (or with 429 if the rate limiter rejected).
// status_code is NOT NULL on the table, so 0 is the in-flight sentinel.
async function insertInFlight(agent_id, entry) {
  try {
    const { data, error } = await supabase.from('agent_audit_log').insert([{
      agent_id,
      method:       entry.method,
      path:         entry.path,
      status_code:  0,
    ...
    if (error) { console.error('Pre-audit insert failed:', error.message); return null }
    return data?.id || null
  } catch (e) {
    console.error('Pre-audit insert threw:', e?.message || e)
    return null
  }
}

async function finalizeInFlight(id, status_code, response_body) {
  if (!id) return
  try {
    const { error } = await supabase.from('agent_audit_log')
      .update({ status_code, response_body: response_body ?? null })
      .eq('id', id)
    if (error) console.error('Audit finalize failed:', error.message)
  } catch (e) {
    console.error('Audit finalize threw:', e?.message || e)
  }
}
...
export function withAgentAuth(handler) {
  return async function wrapped(req, res) {
    ...
    const auditId = await insertInFlight(agent_id, {
      method:       req.method,
    ...
    if (!underLimit) {
      ...
      await finalizeInFlight(auditId, 429, body)
    ...
    await finalizeInFlight(auditId, capturedStatus, capturedBody)
  }
}
